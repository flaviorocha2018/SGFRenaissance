...
namespace SGFRenaissance
{
    public partial class TelaStatusTituloReceber : Form
    {
        ...
        private void status_Titulos_a_ReceberDataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 2)
            {
                ((EntradaTitulosaReceber)Owner).descricaoStatusTitulo.Text = status_Titulos_a_ReceberDataGridView.Rows[e.RowIndex].Cells[1].Value.ToString();
            ...
            else
            {
                if (e.ColumnIndex == 3)
                {
                   ((EntradaTitulosaReceber)Owner).descricaostatusparcela.Text = status_Titulos_a_ReceberDataGridView.Rows[e.RowIndex].Cells[1].Value.ToString();
                   ((EntradaTitulosaReceber)Owner).cod_Status_TituloParcela.Text = status_Titulos_a_ReceberDataGridView.Rows[e.RowIndex].Cells[0].Value.ToString();
                    this.Close();
                }
            }
